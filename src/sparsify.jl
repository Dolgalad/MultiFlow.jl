...
Abstract type shared by all Sparsifier models. These are objects who implement the [`sparsify(sprs::AbstractSparsifier, pb::MCF)`](@ref) that returns a `(ne(pb), nk(pb))` 0-1-matrix where if `M[a,k]==1` arc ``a`` is classifier as being used to route demand ``k``
"""
...
function sparsify(pb::MCF{T,N}, sprs::SPSparsifier; dstmx::AbstractMatrix=cost_matrix(pb)) where {T,N}
    ...
    for (k,demand) in enumerate(pb.demands)
        if (demand.src,demand.dst) in keys(done_endpoints)
            push!(I, I[done_endpoints[demand.src,demand.dst]])
            push!(J, k * ones(Int64, size(I[k],1)))
        else
            ...
            if sprs.k==1
                push!(I, edge_indices(pths, pb.graph))
                push!(J, k*ones(Int64, size(I[k],1)))
            else
                eidx = vcat([edge_indices(p,pb.graph) for p in pths]...)
                push!(I, eidx)
                push!(J, k*ones(Int64, size(eidx,1)))
            end
        end
    end
    I,J = vcat(I...), vcat(J...)
    return sparse(I,J,ones(Bool,size(I,1)), ne(pb), nk(pb))
end
